...
char	g_sTeamColors[][]		= {"\x07B2B2B2", "\x07B2B2B2", "\x07FF4040", "\x07FFFF99"};
...
public Plugin myinfo = {
	...
	author = "Phil25 - edited by bryson",
...
public APLRes AskPluginLoad2(Handle hMyself, bool bLate, char[] sError, int iErrorSize){
	...
	if(!StrEqual(sGame, "open_fortress")){
		Format(sError, iErrorSize, CONS_PREFIX ... " This plugin only works for Open Fortress.");
...
public void OnPluginStart(){
	...
	g_hCvarLog					= CreateConVar("sm_rtd2_log",			"1",		DESC_LOG,					FLAGS_CVARS);
	...
	g_hCvarInSetup				= CreateConVar("sm_rtd2_insetup",		"1",		DESC_IN_SETUP,				FLAGS_CVARS, true, 0.0, true, 1.0);
...
void PrintToNonRollers(int client, Perk perk, int iDuration){
	...
	if(!g_bCvarShowTime || perk.Time == -1)
		RTDPrintAllExcept(client, "%T",
			...
			g_sTeamColors[GetClientTeam(client) - 2],
			sRollerName,
			0x01,
			perk.Good ? PERK_COLOR_GOOD : PERK_COLOR_BAD,
			sPerkName, 0x01);
...
void PrintPerkEndReason(int client, RTDRemoveReason reason=RTDRemove_WearOff, const char[] sCustomReason=""){
	...
	if(g_iCvarChat & CHAT_REMOTHER)
		RTDPrintAllExcept(client, "%T", sReasonOthers, LANG_SERVER, g_sTeamColors[GetClientTeam(client) - 2], client, 0x01, reason == RTDRemove_Custom ? sCustomReason : "");
}
...
void DisplayPerkTimeFrame(client){
	int iTeam	= GetClientTeam(client) - 1;
